...
const BookingScreen = ({ route, navigation }) => {
    ...
    if (service && selectedSize) {
      ...
      let addonsPrice = selectedAddons.reduce((sum, addonId) => {
        ...
        return (
          sum +
          (addon
            ? addon.price || (addon.priceRange ? addon.priceRange[0] : 0)
            : 0)
        );
      }, 0);
  ...
  const handleBooking = () => {
    if (
      APARTMENT_SIZES[selectedSize] === "Larger Homes/Duplexes" &&
      typeof service.pricing[APARTMENT_SIZES[selectedSize]] !== "number"
    ) {
      Alert.alert(
        "Custom Quote Required",
        "For larger homes, please contact us for a custom quote. You can find contact details in the 'Profile' or 'Why Choose Us' section."
      );
      return;
    }
    ...
    const bookingDetails = {
      ...
      addons: selectedAddons.map(
        (id) => ADDON_SERVICES.find((a) => a.id === id)?.name
      ),
      totalPrice,
    };
      ...
      <ScrollView contentContainerStyle={styles.container}>
        ...
        >
          ...
          <Text style={styles.infoText}>
            Note: DateTimePicker needs specific setup per platform. This is a
            placeholder for its UI.
          </Text>
        </Animatable.View>
            ...
            >
              ...
              <Text style={styles.addonPrice}>
                {addon.price
                  ...
                  : `₦${addon.priceRange[0].toLocaleString()} - ₦${addon.priceRange[1].toLocaleString()}`}
              </Text>
            </TouchableOpacity>
          ))}
        </Animatable.View>
